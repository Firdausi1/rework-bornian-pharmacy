const physicians = [
  { name: 'Dr. Samson', availableTimes: ['09:00', '11:00', '14:00'] },
  { name: 'Dr. John', availableTimes: ['10:00', '13:00', '15:30'] },
  { name: 'Dr. Esther', availableTimes: ['08:30', '12:00', '16:00'] }
];
...
const symptomsDiagnosis = {
  ...
  'stomach ache, nausea, vomiting': { diagnosis: 'Food Poisoning', medication: 'Antacid' }
};
...
window.onload = function() {
  ...
  physicians.forEach((physician, index) => {
      ...
      option.value = index;
      option.textContent = `${physician.name} (Available: ${physician.availableTimes.join(', ')})`;
...
function getDiagnosis() {
  ...
  const result = Object.keys(symptomsDiagnosis).find(key => symptoms.includes(key));
  if (result) {
      ...
      diagnosisResult.textContent = 'No matching diagnosis found. Please consult a physician.';
  }
